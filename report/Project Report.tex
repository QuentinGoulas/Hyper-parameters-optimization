...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \draw (-3.13,7.25) node(input) [draw]{\begin{tabular}{c} Data, Seed,\\ Method,\\ Method parameters\end{tabular}};
        \node[matrix, draw, column sep =0cm, inner sep = 0.75cm, rounded corners =4pt] (hpo) at (1,4)
        {
            \node[rectangle, draw] (object) at (0,0) {\begin{tabular}{c}Hyper Parameter\\ Optimizer\end{tabular}}; & \node at (0.5,0) {\small{Iterate till convergence}}; & \node[rectangle, draw] (model) at (1,0) {DL Model};\\
        };
        \draw (-3.13,1) node(output) [draw] {\begin{tabular}{c}Best Hyperparameter\\ configuration\end{tabular}};
        ...
        \draw [-Latex, rounded corners=4pt] (model.north) |- node[above] {train\_model} (7.9,5.25) |- (model.east);
...
Model trainings for this project have been entirely done on local GPUs (GeForce MX230 and GeForce \textbf{GPU DE KINTINE}) with a batch of size 64. Training the classifier on CIFAR-10 as provided by \verb+torchvision+ led to an epoch duration from 20 to 40s depending on model complexity.
