...
The source code for this project can be found at this
\href{https://github.com/QuentinGoulas/Hyper-parameters-optimization.git}{repository
        [link]}
...
Grid Search, Random Search, Bayesian Optimizer Hyper Band and Particle Swarm
Optimization (said to be the best hyperparameter optimizing method in the
current literature)

\section{Algorithm descriptions}
We describe here the algorithms that have been implemented throughout this
project : Grid search, Random search, Particle swarm optimization. We also
provide the pseudo-codes for the methods.

\subsection{Grid search}
This method consists in ``brute forcing'' the best hyperparameter
configuration, i.e by trying all possible hyperparameter configurations on the
hyperparameter space and returning the best configuration. This method is
extremely computationnaly heavy, with the number of possible configurations
increasing exponentially as the number of tested hyperparameters increases.
\begin{algorithm}
    \SetAlgoLined
...
This method consists in randomly selecting a proportion of all the possible
hyperparameter configurations and returning the best configuration after trying
all configurations in the sample. This method reduces slightly the
computational time compared to grid search. However, this method does not solve
the curse of dimensionality problem. As a matter of fact, the only method to
solve this problem is to use a sampling proportion of the order $p^d$ with the
$d$ the number of hyperparameters. This technique keeps computation time
constant but dramatically reduces the exhaustivity of the method and success
...
algorithm which assumes next to no information on the functions to optimize.
Modelled on the evolution of bird flocks, it consists in evaluation the model
accuracy on a small set of hyperparameter configurations, then choosing a new
set of configurations on the basis of the best configuration seen overall and
by the best configuration seen by the particles individually.
...
The convergence of the Particle Swarm Optimization algorithm cannot be proven
in the general case (although some refinements can provide convergence to a
local optimum \cite{engelbrechtConvergenceProofParticle2010}). Yet, empirical
data shows the efficiency of this algorithm in a majority of cases.

\subsection{Genetic Algorithms}
The genetic algorithm is a metaheuristic optimization algorithm inspired by the
process of natural selection. It uses the concepts of selection, crossover and
mutation to evolve a population of individuals towards the best solution. The
algorithm starts by generating an initial population of individuals, then
iteratively selects the best individuals, crosses them over to create new
individuals and mutates them to introduce diversity in the population. The
algorithm stops when a stopping criterion is met, such as a maximum number of
generations or a convergence criterion. In our case each individual represents
a hyperparameter configuration and the fitness of an individual is the accuracy
of the model trained with this configuration. It is a similar algorithm to PSO
but with a different methodology to evolve the population. The pseudo-code of
the genetic algorithm is given below.

\begin{algorithm}[H]
    \SetAlgoLined
    \caption{Genetic Algorithm (GA) algorithm}
    \KwIn{Hyperparameter space $H$, training set $D$, validation set $V$, population size $S$, $n_b$ number of best individual selected, $n_r$ number of random individual selected, mutation rate $p_m$, crossover rate $p_c$, number of generations $G$}
    \KwOut{Best hyperparameter configuration $h^*$ and accuracy $a^*$}
    \BlankLine
    Initialize the population of size $S$ with random hyperparameter configurations\;
    \For{$g \leftarrow 1$ \KwTo $G$}{
        Evaluate the fitness (accuracy) of each individual in the population\;
        Select $n_b$ best individuals and $n_r$ random individuals to form the parents of the next generation\;
        Apply crossover with probability $p_c$ to create new individuals\;
        Apply mutation with probability $p_m$ to introduce diversity in the population\;
    }
    \Return{Best individual in the population (best hyperparameter configuration and accuracy)}
\end{algorithm}
\vspace{5pt}

\textbf{Selection step}

The selection step is used to select the best individuals in the population to
form the next generation. There are several methods to perform the selection
step, but here we choose a number $n_b$ of the best individuals in the
population based on their accuracy and a number $n_r$ of random individuals to
form the parents of the next generation. The random individuals are chosen in
order to keep diversity in the population and not fall into a local optimum.

\vspace{5pt}

\textbf{Crossover step}

The crossover step is used to create new individuals from the parents. In our
case, we use a uniform crossover method where two parents give two children by
exchanging their hyperparameters with a certain probability $p_c$. This assure
a local exploration of the hyperparameter space by similar and accurate
individuals but also a more global exploration by the random individuals.

\vspace{5pt}

\textbf{Mutation step}

The mutation step is used to introduce diversity in the population. It consists
in choosing with a probability $p_m$ a hyperparameter of an individual and
changing it to a random value in the hyperparameter space. This assures that we
theoratically explore the whole hyperparameter space if we compute enough
generations but also that we introduce randomized diversity in the
population.\\

One must carefully choose the values of $S$, $n_b$, $n_r$, $p_m$ and $p_c$ in
order to assure a fast convergence to a good solution. The algortihm proposed
and implemented in this project is a simple genetic algorithm, but there are
many variations of genetic algorithms that can be very useful in the case of
hyperparameter optimization. In our case, we will use a population size of
around 10 individuals which is not sufficient to have a fine accuracy because
we won't have enough diversity, but we are limited by computation time. A
solution to this problem would be to increase the population size, use a more
complex crossover method, or use an optimization method more appropriate to
local search.
...
hyperparameters of interest are the output sizes of the convolutional layers
(C1, C3, C5) and the output size of the first dense layer (F6)
...
The accuracy function of the classifier is chosen as the proportion of accurate
classifications from the model on a validation dataset.
...
To try out the different hyperparameter optimizers, we search for the optimal
configurations of a LeNet-5 type network
\cite{lecunGradientbasedLearningApplied1998}. More precisely, we look for the
optimal number of filters for the three convolutional layers C1, C3 and C5 as
well as the size of the network's dense layer F6. A schematic of the LeNet-5
architecture is provided in Figure \ref{fig:lenet}.
...
We use the LeNet-5 architecture to recognize different images from the CIFAR-10
dataset \cite{krizhevskyLearningMultiplyeLayers2009}, divided into 10
categories : \{ airplane, automobile, bird, cat, deer, dog, frog, horse, ship,
truck\}. The CIFAR-10 comprises $60\ 000$ coloured images of $32\times32$
pixels with $6\ 000$ images per category and split into a $50\ 000$ image
training set and a $10\ 000$ image test set.

Model trainings for this project have been entirely done on local GPUs (GeForce
MX230 and GeForce RTX3070 Laptop) with a batch of size 64. Training the
classifier on CIFAR-10 as provided by \verb+torchvision+ led to an epoch
duration from 20 to 40s depending on model complexity.
...
\begin{figure}[h]
    ...
    \begin{tabular}{|c|c|}
        ...
        Genetic                     & $S=8, n_b=4, n_r=1, p_m=0.1, p_c=0.5, G=10, epoch=10$         \\
        \hline
    \end{tabular}
...
Unless indicated otherwise, the number of epochs for every trainings is set at
40. The probability parameter $p$ of Random Search is chosen so as to consume a
similar number of epochs as PSO and BOHB, which is around 4000 epochs per
optimisation run.
...
We select this hyperparameter space by using the initial hyperparameter
configuration used in LeNet-5 and explore neighbouring configurations to it.

Although the implemented algorithms enable increased efficiency compared to a
babysitting or grid search approach, each optimization run took important
amounts of time, allowing minimal repetitions to validate the results, leaving
significant space to interpretation of the results and little quantitative
results. We still present the results in the following section.

\section{Results}

Running the algorithms returns us the following results in
Figure~\ref{tab:results}. Due to the extreme resource consumption of grid
search ($\sim 740\ 000\ 000$ epochs and $\sim 6\ 000\ 000$ compute hours), we
weren't able to do a full run for this algorithm.
\begin{figure}[h]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|}
        ...
        Genetic                    & $ 298 $                                    & $32$                                                                        & $32$                                                                  & $125$ & $10\times 8\times 10$                        & $68.9\%$ \\ \hline
...
From these results, we observe that both PSO, genetic, random search and BOHB
provide good solutions for hyperparameter optimization, with significantly
reduced costs compared to Grid Search or Random Search. From this experiment,
there is no clear way of knowing which of BOHB, PSO or genetic is best to solve
this specific hyperparameter problem (whether ``best'' is defined by
convergence speed or reliability). Maybe a combination of these algorithms
could be used to improve the results: for example using PSO to explore the
hyperparameter space and then using BOHB to refine the search, or using a
random search on a very large amount of configurations to find a good starting
point for a genetic algorithm.\\

At the end of this project, we can conclude that the implemented algorithms
works well for the hyperparameter optimization of a LeNet-5 model on the
CIFAR-10 dataset. The results obtained are satisfying and show that the
algorithms are able to improve hyperparameter configurations after each
iteration. However, the results are not sufficient to conclude on the best
algorithm to use for hyperparameter optimization. The choice of the algorithm
to use will depend on the specific problem to solve and the resources
available.
